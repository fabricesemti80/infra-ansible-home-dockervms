...
ZONE_ID="${ZONE_ID:-your-zone-id}"
DOMAIN="${DOMAIN:-external.example.com}"
TUNNEL_ID="${TUNNEL_ID:-your-tunnel-id}"
...
if [[ -z "$CLOUDFLARE_API_TOKEN" || -z "$ZONE_ID" || -z "$DOMAIN" || -z "$TUNNEL_ID" ]]; then
  log "❌ Error: Missing required environment variables."
  exit 1
fi

# Fetch existing DNS record
RESPONSE=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$ZONE_ID/dns_records?type=CNAME&name=$DOMAIN" \
